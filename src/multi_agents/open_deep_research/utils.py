...
# SERPAPI Search Tool
# =======================
SERPAPI_SEARCH_DESCRIPTION = (
    "Google results via SerpAPI. Returns titles, URLs, and snippets; "
    "supports tbm=news/images/videos and localization."
)

def _get_serpapi_key(config: Optional[RunnableConfig]) -> Optional[str]:
    if os.getenv("GET_API_KEYS_FROM_CONFIG", "false").lower() == "true" and config:
        return (config.get("configurable", {}) or {}).get("apiKeys", {}).get("SERPAPI_API_KEY")
    return os.getenv("SERPAPI_API_KEY")

async def _serpapi_fetch(session: aiohttp.ClientSession, params: Dict[str, Any]) -> Dict[str, Any]:
    async with session.get("https://serpapi.com/search.json", params=params, timeout=60) as r:
        r.raise_for_status()
        return await r.json()

def _serp_to_flat_items(data: Dict[str, Any], tbm: Optional[str]) -> List[Dict[str, str]]:
    ...
    def add(title: str, url: str, snippet: str):
        if url:
            items.append({
                ...
                "url": url,
                "content": snippet or "",
                "raw_content": snippet or ""
            })

    for o in data.get("organic_results", []) or []:
        add(o.get("title"), o.get("link") or o.get("redirect_link"), o.get("snippet") or "")

    for n in data.get("news_results", []) or []:
        add(n.get("title"), n.get("link"), n.get("snippet") or n.get("source") or "")

    for m in data.get("images_results", []) or []:
        add(m.get("title"), m.get("original") or m.get("link") or m.get("thumbnail"), m.get("source") or "")

    for v in data.get("video_results", []) or []:
        add(v.get("title"), v.get("link"), v.get("snippet") or v.get("platform") or "")

    kg = data.get("knowledge_graph") or {}
    if kg.get("title") and kg.get("source", {}).get("link"):
        add(kg["title"], kg["source"]["link"], kg.get("description") or "")

    return items

async def serpapi_search_async(
    search_queries: List[str],
    max_results: int = 10,
    topic: Literal["general", "news", "images", "videos"] = "general",
    ...
    api_key = _get_serpapi_key(config)
    if not api_key:
        return []

    tbm = None
    if topic == "news": tbm = "nws"
    elif topic == "images": tbm = "isch"
    elif topic == "videos": tbm = "vid"
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async def run_one(q: str):
            ...
            data = await _serpapi_fetch(session, params)
            return {"query": q, "results": _serp_to_flat_items(data, tbm)}

        return await asyncio.gather(*[run_one(q) for q in search_queries])

@tool(description=SERPAPI_SEARCH_DESCRIPTION)
async def serpapi_search(
    queries: List[str],
    max_results: Annotated[int, InjectedToolArg] = 10,
    topic: Annotated[Literal["general","news","images","videos"], InjectedToolArg] = "general",
    location: Annotated[Optional[str], InjectedToolArg] = None,
    gl: Annotated[Optional[str], InjectedToolArg] = None,
    hl: Annotated[Optional[str], InjectedToolArg] = None,
    safe: Annotated[Optional[str], InjectedToolArg] = None,
    tbs: Annotated[Optional[str], InjectedToolArg] = None,
    start: Annotated[int, InjectedToolArg] = 0,
    config: Optional[RunnableConfig] = None,
) -> str:
    """
    Enhanced SerpAPI search:
    - Expands queries for broader coverage.
    - Paginates through results.
    - Fetches & cleans full HTML from result URLs.
    - Summarizes with richer content for deeper reasoning.
    """
    api_key = _get_serpapi_key(config)
    if not api_key:
        return "No SERPAPI_API_KEY found."
    ...
    headers = {"Accept": "application/json"}
    uniques: Dict[str, Dict[str, str]] = {}

    async with aiohttp.ClientSession(headers=headers) as session:

        async def run_one(q: str, start_idx: int):
            params = {
                "engine": "google",
                "q": q,
                "api_key": api_key,
            ...
            data = await _serpapi_fetch(session, params)
            for r in _serp_to_flat_items(data, tbm):
                if r["url"] not in uniques:
                    uniques[r["url"]] = {**r, "query": q}

        # Run each query twice (pagination: start=0 and start=max_results)
        tasks = []
        for q in expanded_queries:
            tasks.append(run_one(q, 0))
            tasks.append(run_one(q, max_results))
        await asyncio.gather(*tasks)

    # Fetch & clean HTML content for richer summaries
    ...
    html_contents = await asyncio.gather(*[fetch_html(u) for u in uniques.keys()])
    for url, html in zip(list(uniques.keys()), html_contents):
        if html:
            uniques[url]["raw_content"] = html

    # Summarize using your existing summarizer
    from .configuration import Configuration
    from .llm import init_chat_model, get_api_key_for_model
    from .schemas import Summary
    from .summarize import summarize_webpage

    cfg = Configuration.from_runnable_config(config)
    model_key = get_api_key_for_model(cfg.summarization_model, config)
    summarizer = (
        init_chat_model(
    ...
    max_char = cfg.max_content_length
    tasks = [
        summarize_webpage(summarizer, (r.get("raw_content") or r.get("content") or "")[:max_char])
        if (r.get("raw_content") or r.get("content")) else asyncio.sleep(0, result=None)
        for r in uniques.values()
    ]
    summaries = await asyncio.gather(*tasks)

    # Format output
    out = "Search results:\n\n"
    for i, (url, r, s) in enumerate(zip(uniques.keys(), uniques.values(), summaries)):
        out += f"\n\n--- SOURCE {i+1}: {r.get('title') or url} ---\n"
        out += f"URL: {url}\n\nSUMMARY:\n{(s or r.get('content') or '')}\n"
        out += "\n" + "-"*80 + "\n"

    return out if uniques else "No results found."
...
async def get_search_tool(search_api: SearchAPI):
    if search_api == SearchAPI.ANTHROPIC:
        ...
        t = tavily_search
        t.metadata = {**(t.metadata or {}), "type": "search", "name": "web_search"}
        return [t]
    elif search_api == SearchAPI.SERPAPI:
        t = serpapi_search
        t.metadata = {**(t.metadata or {}), "type": "search", "name": "web_search"}
        return [t]
